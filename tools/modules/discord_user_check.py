...
DISCORD_USERS_RATE_LIMIT = int(os.getenv("DISCORD_USERS_RATE_LIMIT", 10))  # Requests per minute
...
def check_discord_username(discord_id, expected_username):
    ...
    if response.status_code == 200:
        ...
        if current_username == expected_username:
            log_message(f"Username for Discord ID {discord_id} is correct: {expected_username}")
            return None  # Username is correct, no update needed

        elif 'deleted_user' in current_username:
            log_message(f"User with Discord ID {discord_id} has been deleted. Updating status.")
        ...
        log_message(f"Error retrieving user data for Discord ID {discord_id}: {response.status_code}")
        return None
...
def main():
    ...
    try:
        ...
        if not DISCORD_TOKEN:
            log_message("DISCORD_TOKEN not found in the environment variables. Exiting.")
            return

        # Print the rate limit
        log_message(f"Rate limit found: {DISCORD_USERS_RATE_LIMIT} requests per minute.")
        ...
        for account_number, account_data in data.items():
            ...
            log_message(f"Checking account {account_number}: {discord_id} ({expected_username})")

            # Get the updated username or account status
            updated_username_or_status = check_discord_username(discord_id, expected_username)
